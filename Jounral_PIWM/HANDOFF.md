...
>    (0.463 → 0.452 → 0.319),基线单调**变差**(V2P 0.704 → 0.919 → 0.938)。
>    **应该从"局限"搬到"结果"。**
>
>    ⚠️ **但措辞有硬约束**(`piwm/HANDOFF.md` §0.12 末节):同量级高斯对照给出**同样的收益**
>    (0.313 ± 0.043 vs biased-uniform 的 0.319 ± 0.049,逐折配对差 −0.005 m 且符号不一致),
>    所以**不能写成"弱监督帮了我们"** —— 会议版那个 biased-uniform 噪声模型没有任何特殊性。
>    正确写法:*该量级的标签噪声对结构化模型起正则化作用(任何同量级零均值噪声皆可),
>    而对所有基线都是损害;发现在于这个不对称,不在噪声的形式。*
>    量化:同样 δ=10%,我方 −31%(0.463→0.319),V2P **+33%**(0.704→0.938)。
>    这样写反而更强:是关于**模型类**的结论,不依赖特定噪声模型。
...
> 原因**。
>
> ❌ **补跑 `goku_obs` 修不了这个**(`piwm/HANDOFF.md` §0.11):`DynamicsGOKULane(theta_dim=8)`
> 与 `DynamicsVid2ParamLane` 是**同一个架构**,同种子下训出的权重**逐位相同**——它就是
> Vid2Param 换了个名字,不能作为独立基线行写进论文。真要修需要实现 GokuNet **自己的**
> 结构(latent ODE + 从观测推断初值与参数),那是一个新基线,不是配置项。
>
> **所以正文目前应如实披露:唯一能看图像的基线是 Vid2Param。** 顺带一提,这一发现也说明
> §0.3 里"goku_obs 0.561 vs v2p 0.384"那条不是选择准则的证据,而是训练种子方差
> ——引用它时要改口径。
>
