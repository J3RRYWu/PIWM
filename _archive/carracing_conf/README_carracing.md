...
**Lane propagation.** The dynamics block keeps the lane vector aligned with the car frame across many roll-out steps via a 4-step "rigid → resample → extrapolate → residual" recipe — the resample step is the key fix that prevents semantic drift over a 100-step rollout. Visualised with concrete numbers (Δs = 5, Δy_b = 1.5, α = 0.3):

![Resample step](reports/figures/lane_resample.png)

A self-contained Chinese-language walkthrough of the lane mechanism — symbol glossary, derivation of every step, worked numerical example, and a discussion of training-vs-inference fairness — is shipped as a PDF: [reports/figures/lane_prediction_cn.pdf](reports/figures/lane_prediction_cn.pdf) (regenerate with `python reports/make_lane_doc_cn.py`).
